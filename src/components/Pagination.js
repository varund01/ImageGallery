...
function Pagination() {
    ...
    const getTotalNumberOfPages = () => {
        ...
        axios({
            url : "http://localhost:7800/getSize",
            method : "post",
            headers: {
    ...
    const getImagesByPage = (curr) => {
        ...
        axios({
            url : "http://localhost:7800/getImagesByPage",
            method : "post",
            headers: {
                "Content-type":"application/x-www-form-urlencoded",
                "Token":localStorage.getItem("Token"),
            ...
            <div>
                {
                    ...
                    imagesByPage.map((post,index) => (
                            <Post key={index} posts={imagesByPage} setPosts={setImagesByPage} setSelectedImg={setSelectedImg} caption={post.caption} category={post.category} imageUrl={`assets/${post.imageUrl}`}/>
                        ))
